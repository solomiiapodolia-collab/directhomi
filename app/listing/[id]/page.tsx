...
const heatingLabels: Record<string, string> = {
  ...
  individual: "Індивідуальне опалення",
  none: "Без опалення",
};
        ...
        <main className="flex flex-1 items-center justify-center">
          <div className="text-center">
            ...
            <p className="mt-2 text-muted-foreground">
              Це оголошення може бути видалене або недоступне
            </p>
  ...
  return (
    <div className="flex min-h-screen flex-col">
            ...
            <div className="space-y-6 lg:col-span-2">
              ...
              <div className="relative overflow-hidden rounded-xl">
                <div className="relative aspect-[16/10]">
                  <Image
                    ...
                    alt={`${listing.title} - фото ${currentPhotoIndex + 1}`}
                    fill
              ...
              <div>
                ...
                <div className="mt-2 flex items-center gap-1 text-muted-foreground">
                  ...
                  <span>
                    {listing.address.city}, {listing.address.district}, {listing.address.street} {listing.address.buildingNumber}
                    {listing.address.apartmentNumber && `, кв. ${listing.address.apartmentNumber}`}
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-6 rounded-lg bg-muted/50 p-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium">{listing.features.rooms} кімн.</span>
                </div>
                <div className="flex items-center gap-2">
                  <Maximize className="h-5 w-5 text-muted-foreground" />
                  <span className="font-medium">{listing.features.area} м²</span>
                </div>
                {listing.address.floor && (
                  <div className="flex items-center gap-2">
                    <span className="text-muted-foreground">Поверх:</span>
                    <span className="font-medium">
                      {listing.address.floor} з {listing.address.totalFloors}
                    </span>
                  </div>
                )}
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-muted-foreground">
                    Додано {formatDate(listing.createdAt)}
                  </span>
                </div>
              </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="font-medium">{conditionLabels[listing.features.condition]}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <ThermometerSun className="h-5 w-5 text-muted-foreground" />
                      <div>
                        <p className="text-sm text-muted-foreground">Опалення</p>
                        <p className="font-medium">{heatingLabels[listing.features.heatingType]}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    ...
                    {listing.features.hasFurniture && (
                      <Badge variant="outline" className="gap-1">
                        <Sofa className="h-3 w-3" />
                        З меблями
                      </Badge>
                    )}
                    {listing.features.hasAppliances && (
                      <Badge variant="outline" className="gap-1">
                        З технікою
                      </Badge>
                    )}
                  ...
                  <div className="space-y-2">
                    ...
                    <a
                      ...
                      download="DirectHomi-договір-оренди.pdf"
                    ...
                    <p className="text-xs text-muted-foreground text-center">
                      DirectHomi не є стороною угоди. Договір укладається напряму між власником та орендарем.
                    </p>
                  </div>
                </CardContent>
              </Card>
      ...
      <SiteFooter />
                    ...
                    <div className="flex flex-wrap gap-3">
                      <Badge
                        variant={listing.features.petsAllowed ? "default" : "secondary"}
                        className={cn(
                          "gap-1",
                          listing.features.petsAllowed ? "bg-emerald-500" : "bg-destructive/10 text-destructive"
                        )}
                      >
                        <Dog className="h-3 w-3" />
                        {listing.features.petsAllowed ? "Можна з тваринами" : "Без тварин"}
                      </Badge>
                      <Badge
                        variant={listing.features.childrenAllowed ? "default" : "secondary"}
                        className={cn(
                          "gap-1",
                          listing.features.childrenAllowed ? "bg-emerald-500" : "bg-destructive/10 text-destructive"
                        )}
                      >
                        <Baby className="h-3 w-3" />
                        {listing.features.childrenAllowed ? "Можна з дітьми" : "Без дітей"}
                      </Badge>
                      <Badge
                        variant={listing.features.smokingAllowed ? "default" : "secondary"}
                        className={cn(
                          "gap-1",
                          !listing.features.smokingAllowed ? "bg-emerald-500" : "bg-amber-500"
                        )}
                      >
                        <Cigarette className="h-3 w-3" />
                        {listing.features.smokingAllowed ? "Можна палити" : "Не палити"}
                      </Badge>
                    </div>
                  </div>
                </CardContent>
            ...
            <div className="space-y-6">
                ...
                <CardContent className="pt-6">
                  <div className="mb-4">
                    <p className="text-3xl font-bold text-primary">
                      ...
                      {listing.type === "rent" && (
                        <span className="text-lg font-normal text-muted-foreground">/міс</span>
                      )}
                    </p>
                    {listing.deposit && (
                      <p className="mt-1 text-sm text-muted-foreground">
                        Депозит: {formatPrice(listing.deposit)}
                      </p>
                    )}
                    {listing.commission && (
                      <p className="text-sm text-muted-foreground">
                        Комісія: {formatPrice(listing.commission)}
                      </p>
                    )}
                  </div>
                      ...
                      <div>
                        ...
                        {owner && (
                          <p className="text-xs text-muted-foreground">
                            Відповідає на {owner.responseRate}% заявок
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  {canApply ? (
                    ...
                    <Alert className="bg-emerald-50 border-emerald-200">
                      <AlertDescription className="text-emerald-700">
                        Заявку подано! Очікуйте відповіді власника протягом 72 годин.
                      </AlertDescription>
                    </Alert>
                  ) : !user ? (
                    <div className="space-y-2">
                      <Button className="w-full" size="lg" asChild>
                        <Link href="/login">Увійти для подачі заявки</Link>
                      </Button>
                    ...
                    <Alert>
                      ...
                      <AlertDescription>
                        Власники не можуть подавати заявки на оголошення
                      </AlertDescription>
                    </Alert>
                  ) : null}
                  ...
                  <div className="mt-4 flex gap-2">
                    ...
                    >
                      <Heart className={cn("mr-2 h-4 w-4", isSaved && "fill-current text-destructive")} />
              ...
              <Alert>
                ...
                <AlertDescription className="text-xs">
                  DirectHomi не є стороною угоди між вами та власником. 
                  Перевіряйте всі документи та умови перед укладанням договору.
                </AlertDescription>
              </Alert>
            </div>
          </div>
